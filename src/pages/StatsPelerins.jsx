...
function ageFrom(dateStr) {
  ...
  try {
    ...
    if (
      ...
      (m.getMonth() === d.getMonth() && m.getDate() < d.getDate())
    )
      age--;
...
function normalizeSexe(v) {
  ...
  if (!s) return "Non précisé";

  // variantes masculines
  if (["m", "masculin", "homme", "h"].includes(s)) return "Masculin";

  // variantes féminines
  if (["f", "feminin", "féminin", "femme"].includes(s)) return "Féminin";

  // sinon on renvoie la valeur brute
  return v || "Non précisé";
}

/* ====== Palette + logique d'état (bleu-centric) ====== */
const ETAT_COLORS = {
  Complet: {
    bar: "from-emerald-400/90 to-emerald-500/90",
    chip: "bg-emerald-50 text-emerald-700 ring-1 ring-emerald-200",
  },
  "Photo pèlerin manquante": {
    bar: "from-blue-300/90 to-blue-500/90",
    chip: "bg-blue-50 text-blue-700 ring-1 ring-blue-200",
  },
  "Photo passeport manquante": {
    bar: "from-sky-300/90 to-sky-500/90",
    chip: "bg-sky-50 text-sky-700 ring-1 ring-sky-200",
  },
  "N° passeport manquant": {
    bar: "from-indigo-300/90 to-indigo-500/90",
    chip: "bg-indigo-50 text-indigo-700 ring-1 ring-indigo-200",
  },
  "À compléter": {
    bar: "from-rose-300/90 to-rose-500/90",
...
function normalizePelerin(row = {}) {
  return {
    ...
    photoPasseport: mediaURL(
      row.photo_passeport_path || row.photoPasseport || ""
    ),
    ...
    sexe: normalizeSexe(row.sexe || row.gender || ""),
    ...
    numPasseport:
      row.numPasseport || row.num_passeport || row.passeport || "",
    offre: row.offre || "",
    voyage: row.voyage || "",
    ...
    // Meta
    enregistrePar:
      row.created_by_name || row.createdByName || row.createdBy || "—",
...
export default function StatsPelerins() {
  ...
  const filtered = useMemo(() => {
    ...
    const arr = rows.filter((p) =>
      year === "all" ? true : String(p.anneeVoyage) === String(year)
    );
    if (!q) return arr;
    return arr.filter((p) =>
      [
  ...
  /* Agrégations */
  const bySexe = useMemo(
    () => countBy(filtered, (x) => x.sexe || "Non précisé"),
    [filtered]
  );
  const byOffre = useMemo(
    () => countBy(filtered, (x) => x.offre || "—"),
    [filtered]
  );
  const byVoyage = useMemo(
    () => countBy(filtered, (x) => x.voyage || "—"),
    [filtered]
  );
  const byYear = useMemo(
    () => countBy(filtered, (x) => x.anneeVoyage || "—"),
    [filtered]
  );
  const byEtat = useMemo(() => countBy(filtered, dossierState), [filtered]);

  const byResidence = useMemo(() => {
    const m = countBy(filtered, (x) =>
      cityFrom(x.adresse, x.urgenceResidence)
    );
  ...
  const ages = filtered
    .map((x) => ageFrom(x.dateNaissance))
    .filter((a) => a !== null);
  const avgAge = ages.length
    ? Math.round(
        (ages.reduce((s, a) => s + a, 0) / ages.length) * 10
      ) / 10
    : "—";
  ...
  const passeportNumOk = filtered.filter(
    (x) => !!(x.numPasseport || "").trim()
  ).length;
  ...
  return (
    <div
      id="stats-print-root"
      className="space-y-6 text-dyn"
      ref={printRef}
    >
      {/* EN-TÊTE (cachée à l'impression) */}
      ...
      <section className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
        ...
        <Kpi
          title="Avec photo"
          value={`${photoOk}/${total}`}
          hint={`${pct(photoOk, total)}%`}
        />
        <Kpi
          title="N° Passeport"
          value={`${passeportNumOk}/${total}`}
          hint={`${pct(passeportNumOk, total)}%`}
        />
        <Kpi
          title="Dossier complet"
          value={`${completeCount}/${total}`}
          hint={`${completenessPct}%`}
        >
      ...
      {/* Graphs — responsive, tous IMPRIMABLES */}
      <section className="grid gap-4 lg:grid-cols-2">
        <Card title="Répartition par sexe">
          <Bars data={bySexe} total={total} />
        </Card>

        <Card title="Répartition par offre">
          <Bars data={byOffre} total={total} />
        </Card>

        <Card title="Types de voyage">
          <Bars data={byVoyage} total={total} />
        </Card>

        <Card title="Inscriptions par année">
          <Bars data={byYear} total={total} />
        </Card>

        <Card title="Répartition par résidence (ville)">
          <Bars data={byResidence} total={total} />
        </Card>

        <Card title="Tranches d’âge (estimées)">
          <Bars data={buckets} total={total} />
        </Card>
        ...
        <Card title="Top 5 — Agents (inscriptions)">
          <TopList
            counts={countBy(filtered, (x) => x.enregistrePar || "—")}
            total={total}
            top={5}
          />
        </Card>
      ...
      <style>{`
        ...
        @media print {
          ...
          /* On cache la sidebar + la topbar du layout principal */
          aside, header {
            display: none !important;
          }

          /* On laisse le contenu stats occuper la largeur */
          #stats-print-root {
...
function Kpi({ title, value, hint, children }) {
  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-3 sm:p-4 text-slate-900 shadow-sm print-card">
      <div className="text-[11px] md:text-[12px] uppercase tracking-wider text-slate-500 print:text-black">
        {title}
      </div>
      <div className="mt-1 text-xl md:text-2xl font-extrabold text-blue-700 print:text-black">
        {value}
      </div>
      {hint ? (
        <div className="text-dyn-sm text-slate-500 print:text-black mt-1">
          {hint}
        </div>
      ) : null}
      {children}
...
function Bars({ data, total }) {
  ...
  if (!entries.length)
    return (
      <p className="text-slate-500 print:text-black">Aucune donnée.</p>
    );
  ...
  return (
    <div className="grid place-items-center gap-2">
      ...
      <div className="text-center">
        <div className="text-lg font-extrabold text-blue-700 print:text-black">
          {pct(value, total)}%
        </div>
        <div className="text-dyn-sm text-slate-600 print:text-black">
          {label}
        </div>
      </div>
    </div>
  ...
  return (
    <div>
      <div className="mb-1 flex items-center justify-between text-dyn-sm">
        ...
        <span className="text-blue-700 font-semibold print:text-black">
          {value}%
        </span>
      </div>
      <div className="h-2 w-full rounded-full bg-slate-100 overflow-hidden print:bg-gray-200">
    ...
    <div className="overflow-x-auto">
      <table className="min-w-[520px] text-dyn-sm">
        <tbody className="[&_tr+tr]:border-t [&_tr+tr]:border-slate-200 print:[&_tr+tr]:border-gray-200">
          <tr>
            <td className="py-2 pr-3 text-slate-600 print:text-black">
              Total
            </td>
